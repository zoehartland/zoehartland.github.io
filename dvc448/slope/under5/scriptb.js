	...
	function drawGraphic() {
		...
		if (graphic.width() < threshold_sm) {
			...
			var height = Math.ceil((chart_width * dvc.optional.aspectRatio_sm[1]) / dvc.optional
				.aspectRatio_sm[0]) - margin.top - margin.bottom;
			...
			var chart_width = graphic.width() - margin.left - margin.right;
			var height = Math.ceil((chart_width * dvc.optional.aspectRatio_md[1]) / dvc.optional
				.aspectRatio_md[0]) - margin.top - margin.bottom;
		} else {
			var margin = {
			...
			var chart_width = graphic.width() - margin.left - margin.right;
			var height = Math.ceil((chart_width * dvc.optional.aspectRatio_lg[1]) / dvc.optional
				.aspectRatio_lg[0]) - margin.top - margin.bottom;
		}
		...
		x.domain([d3.min(graphic_data, function(d) {
			return +d.Name;
		...
		function scaleFormat(d, i) {
			if (d > 0) {
				...
				return dvc.essential.leftyRighty[i];
			}
		}
		...
		function insertLinebreaks() {
			...
			var el1 = dvc.essential.legendLabels[i];
			var el = el1.data;
			...
			for (var j = 0; j < words.length; j++) {
				...
				if (j > 0)
					tspan.attr('x', x(dvc.essential.legendLabels[i][0])).attr('dy', '22');
			}
		};
		...
		svg = d3.select('#graphic').append('svg')
			...
			.attr("height", height + margin.top + margin.bottom + 315)
			.append("g")
		...
		svg.append("text")
			...
			.style("padding-bottom", "10px")
			.text(dvc.essential.leftyRighty[0])

		svg.append("text")
			.attr("y", -11)
			.attr("x", chart_width + margin.right - 10)
			.attr("text-anchor", "end")
			.attr("fill", "#666")
			.attr("font-weight", "bold")
			.style("font-size", "11px")
		...
		.text(dvc.essential.leftyRighty[1])
		...
		linegroups = svg.append('g').attr("id", "slopelines").selectAll('path')
			...
			.on("mouseover", function() {
				d3.selectAll(".slopegroup").transition().duration(300).attr("opacity",
					"0.2");
				var thisclass = d3.select(this).attr("class").split(" ", 2)[1];

				d3.selectAll("." + thisclass).transition().duration(300).attr("opacity",
					"1");
		...
		linegroups.append('text')
			...
			.text(function(d, i) {
				return "(" + format1(dvc.essential.actual2015[i]) + ") " +
					dvc.essential
					.legendLabels[i];
			})
			.attr('y', function(d, i) {
		...
		linegroups.append('text')
			...
			.text(function(d, i) {
				return dvc.essential.legendLabels[i] + " (" + format1(dvc.essential.actual1990[
					i]) + ") ";
			})
			.attr('y', function(d) {
		...
		linegroups.forEach(function(d, i) {
			for (var j = 0; j < lines[first].length; j++) {
				...
				linegroupsg.append("text")
					...
					.text(function(d, i) {
						if (j == 1) {
							return dvc.essential.rank90[i]
						} else {
							return dvc.essential.rank15[i]
						}
					})
					.attr("x", function(d, i) {
						return x(d.value[j].Name);
			...
			dvc.essential.annotationChart.forEach(function(d, i) {
				...
				if (dvc.essential.circles == true) {
					...
					svg.append("circle")
						...
						.attr('cx', x(d3.time.format(dvc.essential.dateFormat).parse(dvc.essential
							.annotationCXCY[i][0])))
						.attr("r", "3");
		...
		d3.select(".footer").append("p")
			.append("a")
			.attr("href", dvc.essential.sourceURL2)
			.attr("target", "_blank")
			.html(dvc.essential.sourceText2);	

		d3.select(".footer").append("p")
			.append("a")
			.attr("href", dvc.essential.sourceURL3)
			.attr("target", "_blank")
			.html(dvc.essential.sourceText3);	
	...
	function arrangeGroups() {
		...
		d3.selectAll(".groups0").selectAll('text').attr("transform",
			"translate(-5,0)");
	}
	...
	function arrangeGroups2() {
		...
		d3.selectAll(".groups2").selectAll('text').attr("transform",
			"translate(10,0)");
	}
		...
		d3.json("config.json", function(error, config) {
			...
			d3.csv("data.csv", function(error, data) {
				...
				graphic_data.forEach(function(d) {
					//d.Name = d3.time.format(dvc.essential.NameFormat).parse(d.Name);
